...
const AdminDashboard: React.FC = () => {
  ...
  }, [allSignoffs, totalSkillsPossible, registeredUsers]);
    ...
    <div className="space-y-8 pb-20 animate-fadeIn">
      ...
      <div className="bg-slate-900 text-white p-8 rounded-2xl shadow-xl flex flex-col space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Sign-offs</div>
            <div className="text-2xl font-black">{allSignoffs.length}</div>
          </div>
          <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Advisors</div>
            <div className="text-2xl font-black">{allAdvisors.length}</div>
          </div>
          <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Avg Mastery</div>
            <div className="text-2xl font-black">
              {sortedExplorers.length > 0 
                ? Math.round((allSignoffs.length / (totalSkillsPossible * sortedExplorers.length)) * 100) 
                : 0}%
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4">
        ...
        {/* System Registration Section (Collapsible) */}
        <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
          <button 
            onClick={() => setShowReg(!showReg)}
            className="w-full px-8 py-6 flex justify-between items-center hover:bg-slate-50 transition text-left"
          >
                ...
                <div className="space-y-4">
                  ...
                  <form onSubmit={handleAddUser} className="space-y-3">
                    <input 
                      ...
                      className="w-full px-4 py-3 rounded-xl border outline-none focus:ring-2 focus:ring-red-500 text-sm"
                      value={newUserName}
                    ...
                    <div className="flex space-x-2">
                      <button 
                        type="button" 
                        onClick={() => setNewUserRole(UserRole.EXPLORER)}
                        className={`flex-1 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest border transition-all ${newUserRole === UserRole.EXPLORER ? 'bg-slate-900 text-white border-slate-900 shadow-md' : 'bg-slate-50 text-slate-400 border-slate-200'}`}
                      >
                        Explorer
                      </button>
                      <button 
                        type="button" 
                        onClick={() => setNewUserRole(UserRole.ADVISOR)}
                        className={`flex-1 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest border transition-all ${newUserRole === UserRole.ADVISOR ? 'bg-slate-900 text-white border-slate-900 shadow-md' : 'bg-slate-50 text-slate-400 border-slate-200'}`}
                      >
                        Employee
                      </button>
                    </div>
                    <button type="submit" className="w-full py-4 bg-red-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg hover:bg-red-700 transition">Create Account</button>
                  </form>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="flex space-x-2">
                    <input 
                      type="text" 
                      className="flex-grow px-4 py-3 rounded-xl border outline-none focus:ring-2 focus:ring-red-500 text-sm font-mono"
                      value={sitePassInput}
                      onChange={(e) => setSitePassInput(e.target.value)}
                    />
                    <button onClick={handleUpdateSitePass} className="bg-slate-900 text-white px-6 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-800 transition shadow-lg">Update</button>
                  </div>
                </div>
              </div>
              {mgtMsg && <p className="mt-4 text-center text-[10px] font-black text-green-600 uppercase tracking-widest animate-pulse">{mgtMsg}</p>}
            </div>
          )}
        </div>
