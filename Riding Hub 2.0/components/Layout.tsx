...
import { getCurrentUser, logoutUser, changePassword } from '../services/auth';
...
const ChangePasswordModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleUpdate = () => {
    if (!user) return;
    if (newPass.length < 6) {
      setStatus('Password too short (min 6 chars)');
      return;
    }
    ...
    if (changePassword(user.id, newPass)) {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl space-y-4 animate-fadeIn">
        <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">Update Portal Password</h3>
        ...
        {status && <p className="text-[10px] font-black uppercase text-center text-red-600">{status}</p>}
        <div className="flex space-x-2">
          <button onClick={onClose} className="flex-1 py-3 text-xs font-bold text-slate-400 uppercase">Cancel</button>
          <button onClick={handleUpdate} className="flex-1 py-3 bg-red-600 text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-lg">Update</button>
        </div>
      </div>
    </div>
  );
            ...
            {user && (
              <div className="flex items-center pl-4 border-l border-slate-700 ml-4">
                <div className="hidden lg:block mr-4 text-right">
                  <div className="text-[10px] text-slate-400 uppercase font-bold tracking-widest cursor-pointer hover:text-red-400 transition" onClick={() => setShowPasswordModal(true)}>Settings</div>
